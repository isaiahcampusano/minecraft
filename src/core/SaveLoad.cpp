...
namespace {
constexpr std::array<char,4> MAGIC_V2={'M','C','v','2'},MAGIC_V3={'M','C','v','3'};
...
bool validSlot(const SaveData::SlotData& slot,std::uint32_t version=SaveLoad::VERSION){
  ...
  if(slot.kind==ItemKind::FOOD)return static_cast<std::size_t>(slot.foodType)<static_cast<std::size_t>(FoodType::COUNT)&&slot.count>0&&slot.count<=Inventory::MAX_STACK_SIZE&&slot.durability==0;
...
bool SaveLoad::save(const SaveData& data,const std::filesystem::path& path){
  if(data.selectedSlot<0||data.selectedSlot>=Inventory::HOTBAR_SLOTS||!validSlot(data.cursorStack)||!validSurvival(data)||data.edits.size()>std::numeric_limits<std::uint32_t>::max())return false;
  ...
  out.write(MAGIC_V3.data(),MAGIC_V3.size());const std::uint32_t version=VERSION,editCount=static_cast<std::uint32_t>(data.edits.size());
...
bool SaveLoad::load(SaveData& out,const std::filesystem::path& path){
  ...
  if(!in||!readValue(in,version)||!((magic==MAGIC_V2&&version==2)||(magic==MAGIC_V3&&version==VERSION))||!readValue(in,data.selectedSlot)||data.selectedSlot<0||data.selectedSlot>=Inventory::HOTBAR_SLOTS||!readSlot(in,data.cursorStack,version))return false;
