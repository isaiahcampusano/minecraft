...
void Application::setMenu(MenuState menu){if(menu==m_menu)return;m_menu=menu;m_keybindCapture=-1;m_menuMouseReleaseRequired=true;m_inventoryDragging=false;m_miningAnimation=false;m_useSwingTimer=0;m_sprint.cancel();m_eating.cancel();m_player.resetMiningProgress();m_player.resetMiningCooldown();m_firstMouse=true;m_captured=menu==MenuState::Gameplay;glfwSetInputMode(m_window,GLFW_CURSOR,m_captured?GLFW_CURSOR_DISABLED:GLFW_CURSOR_NORMAL);}
...
void Application::key(GLFWwindow*w,int k,int,int action,int){auto*a=static_cast<Application*>(glfwGetWindowUserPointer(w));if(action!=GLFW_PRESS)return;
  ...
  if(a->m_inventoryOpen){if(k==a->m_settings.key(KeyAction::Inventory))a->setInventoryOpen(false);return;}
...
void Application::mouseButton(GLFWwindow*w,int button,int action,int){
  ...
  if(a->m_inventoryOpen){
    ...
    a->inventoryMouse(static_cast<float>(cursorX*frameWidth/windowWidth),static_cast<float>(cursorY*frameHeight/windowHeight),frameWidth,frameHeight,action);
    return;
  }
  if(a->m_menuMouseReleaseRequired)return;
...
void Application::run(){double last=glfwGetTime();while(!glfwWindowShouldClose(m_window)){double now=glfwGetTime();float dt=std::min(static_cast<float>(now-last),.05f);last=now;m_clickCooldown=std::max(0.f,m_clickCooldown-dt);m_attackCooldown=std::max(0.f,m_attackCooldown-dt);m_useSwingTimer=std::max(0.f,m_useSwingTimer-dt);glfwPollEvents();if(glfwGetMouseButton(m_window,GLFW_MOUSE_BUTTON_LEFT)==GLFW_RELEASE&&glfwGetMouseButton(m_window,GLFW_MOUSE_BUTTON_RIGHT)==GLFW_RELEASE)m_menuMouseReleaseRequired=false;const bool gameplay=m_menu==MenuState::Gameplay;updateSimulation(dt);
    int width,height;glfwGetFramebufferSize(m_window,&width,&height);glViewport(0,0,width,height);glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);glm::mat4 projection=glm::perspective(glm::radians(m_camera.fov),width/static_cast<float>(std::max(height,1)),.1f,500.f);glm::vec3 cameraPos=m_camera.position(m_player,m_world),cameraDir=m_camera.direction(m_player,cameraPos);RayHit target=Raycaster::cast(m_world,m_player.eyePosition(),m_player.forward(),6.f);
    ...
    m_renderer->draw(m_world,m_player,m_mobs,m_camera.isThirdPerson(),gameplay?dt:0.f,m_miningAnimation,m_useSwingTimer,m_camera.view(m_player,cameraPos),projection,cameraPos,target,width,height,hud.str(),m_inventory,m_inventoryOpen,m_tableOpen,m_creativePage,mouseX,mouseY,m_dayNight,static_cast<int>(m_menu),m_world.viewDistance(),m_furnaceOpen?m_world.furnaceAt(m_furnacePosition):nullptr);glfwSwapBuffers(m_window);}}
