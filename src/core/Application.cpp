...
void Application::updateMining(float dt){if(m_inventoryOpen||!m_captured||m_player.targetedBlock.y<=0||glfwGetMouseButton(m_window,GLFW_MOUSE_BUTTON_LEFT)!=GLFW_PRESS){m_player.resetMiningProgress();return;}BlockType type=m_world.getBlock(m_player.targetedBlock.x,m_player.targetedBlock.y,m_player.targetedBlock.z);if(type==BlockType::AIR){m_player.clearMiningTarget();return;}if(m_player.advanceMining(type,dt)){if(m_world.setBlock(m_player.targetedBlock.x,m_player.targetedBlock.y,m_player.targetedBlock.z,BlockType::AIR))m_inventory.add(type);m_player.clearMiningTarget();}}
void Application::run(){double last=glfwGetTime();while(!glfwWindowShouldClose(m_window)){double now=glfwGetTime();float dt=std::min(static_cast<float>(now-last),.05f);last=now;m_clickCooldown=std::max(0.f,m_clickCooldown-dt);glfwPollEvents();m_dayNight.update(dt);m_world.update(m_player.position);input(dt);m_player.update(dt,m_world);
    ...
    m_renderer->draw(m_world,m_player,m_camera.thirdPerson,dt,m_camera.view(m_player),projection,cameraPos,target,width,height,hud.str(),m_inventory,m_inventoryOpen,m_creativePage,m_dayNight);glfwSwapBuffers(m_window);}}
