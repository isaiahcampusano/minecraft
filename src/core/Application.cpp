...
void Application::key(GLFWwindow*w,int k,int,int action,int){auto*a=static_cast<Application*>(glfwGetWindowUserPointer(w));if(action!=GLFW_PRESS)return;if(k>=GLFW_KEY_1&&k<=GLFW_KEY_9)a->m_inventory.select(k-GLFW_KEY_1);else if(k==GLFW_KEY_ESCAPE){a->m_captured=!a->m_captured;a->m_firstMouse=true;glfwSetInputMode(w,GLFW_CURSOR,a->m_captured?GLFW_CURSOR_DISABLED:GLFW_CURSOR_NORMAL);}else if(k==GLFW_KEY_F)a->m_player.toggleFly();else if(k==GLFW_KEY_F5)a->m_camera.togglePOV();}
void Application::mouseButton(GLFWwindow*w,int button,int action,int){auto*a=static_cast<Application*>(glfwGetWindowUserPointer(w));if(button!=GLFW_MOUSE_BUTTON_RIGHT||action!=GLFW_PRESS||!a->m_captured||a->m_clickCooldown>0)return;glm::vec3 origin=a->m_camera.position(a->m_player),direction=a->m_camera.direction(a->m_player);auto hit=Raycaster::cast(a->m_world,origin,direction,a->m_camera.thirdPerson?12.f:6.f);if(!hit.hit||a->m_player.overlapsBlock(hit.adjacent))return;BlockType type=a->m_inventory.selectedType();if(type!=BlockType::AIR&&a->m_inventory.consumeSelected())a->m_world.setBlock(hit.adjacent.x,hit.adjacent.y,hit.adjacent.z,type);a->m_clickCooldown=.12f;}
...
void Application::run(){double last=glfwGetTime();while(!glfwWindowShouldClose(m_window)){double now=glfwGetTime();float dt=std::min(static_cast<float>(now-last),.05f);last=now;m_clickCooldown=std::max(0.f,m_clickCooldown-dt);glfwPollEvents();m_dayNight.update(dt);m_world.update(m_player.position);input(dt);m_player.update(dt,m_world);
    ...
    BlockType selected=target.hit?m_world.getBlock(target.block.x,target.block.y,target.block.z):BlockType::AIR;const char* name="NONE";switch(selected){case BlockType::GRASS:name="GRASS";break;case BlockType::DIRT:name="DIRT";break;case BlockType::BEDROCK:name="BEDROCK";break;case BlockType::STONE:name="STONE";break;case BlockType::COBBLESTONE:name="COBBLESTONE";break;case BlockType::OAK_LOG:name="OAK LOG";break;case BlockType::SAND:name="SAND";break;case BlockType::GRAVEL:name="GRAVEL";break;case BlockType::GLASS:name="GLASS";break;default:break;}
    ...
    m_renderer->draw(m_world,m_player,m_camera.thirdPerson,dt,m_camera.view(m_player),projection,cameraPos,target,width,height,hud.str(),m_inventory,m_dayNight);glfwSwapBuffers(m_window);}}
