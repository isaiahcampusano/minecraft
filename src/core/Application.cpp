...
void Application::mouseButton(GLFWwindow*w,int button,int action,int){auto*a=static_cast<Application*>(glfwGetWindowUserPointer(w));if(action!=GLFW_PRESS||!a->m_captured||a->m_clickCooldown>0)return;glm::vec3 origin=a->m_camera.position(a->m_player),direction=a->m_camera.direction(a->m_player);auto hit=Raycaster::cast(a->m_world,origin,direction,a->m_camera.thirdPerson?12.f:6.f);if(!hit.hit)return;if(button==GLFW_MOUSE_BUTTON_LEFT&&hit.block.y>0){BlockType broken=a->m_world.getBlock(hit.block.x,hit.block.y,hit.block.z);if(a->m_world.setBlock(hit.block.x,hit.block.y,hit.block.z,BlockType::AIR))a->m_inventory.add(broken);}if(button==GLFW_MOUSE_BUTTON_RIGHT&&!a->m_player.overlapsBlock(hit.adjacent)){BlockType type=a->m_inventory.selectedType();if(type!=BlockType::AIR&&a->m_inventory.consumeSelected())a->m_world.setBlock(hit.adjacent.x,hit.adjacent.y,hit.adjacent.z,type);}a->m_clickCooldown=.12f;}
