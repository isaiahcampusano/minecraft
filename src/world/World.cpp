...
void World::markNeighbors(int x,int z,bool lighting){static constexpr int d[4][2]={{1,0},{-1,0},{0,1},{0,-1}};for(const auto& offset:d)if(auto* chunk=find(x+offset[0],z+offset[1])){if(lighting){chunk->markLightingDirty();queueTask(x+offset[0],z+offset[1],ChunkTaskType::UPDATE_LIGHTING);}else{chunk->markDirty();queueTask(x+offset[0],z+offset[1],ChunkTaskType::REBUILD_MESH);}}}
...
bool World::setBlock(int x,int y,int z,BlockType t){if(x<0||x>=1000||z<0||z>=1000||y<0||y>=Chunk::SIZE_Y)return false;int cx=floorDiv(x,16),cz=floorDiv(z,16);loadChunk(cx,cz);auto*c=find(cx,cz);if(!c)return false;c->setBlock(x-cx*16,y,z-cz*16,t);m_edits[{x,y,z}]=t;queueTask(cx,cz,ChunkTaskType::UPDATE_LIGHTING);if(x%16==0||x%16==15||z%16==0||z%16==15)markNeighbors(cx,cz,true);return true;}
...
void World::updateLighting(){for(auto& pair:m_chunks)if(pair.second->lightingDirty())pair.second->computeSkyLight([this](int x,int y,int z){return getBlock(x,y,z);});}
