...
void PlayerPhysics::update(Player&p,float dt,const World&w){const bool wasGrounded=p.onGround;if(p.isFlying||p.gameMode()==GameMode::Creative)p.survival.resetFallDistance();else{p.velocity.y=std::max(p.velocity.y-25.f*dt,-50.f);if(!wasGrounded)p.survival.trackFall(p.velocity.y,dt);}
