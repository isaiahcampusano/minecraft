...
void PlayerPhysics::update(Player&p,float dt,const World&w){const bool wasGrounded=p.onGround;if(p.isFlying)p.survival.resetFallDistance();else{p.velocity.y=std::max(p.velocity.y-25.f*dt,-50.f);if(!wasGrounded)p.survival.trackFall(p.velocity.y,dt);}
  if(p.isSneaking&&wasGrounded&&!p.isFlying&&p.velocity.y<=0&&dt>0){
    float dx=p.velocity.x*dt,dz=p.velocity.z*dt;
    auto supported=[&](float x,float z){return collides(p,p.position+glm::vec3(x,-.05f,z),w);};
    auto reduce=[](float& v){v=std::abs(v)<.01f?0.f:v+(v>0?-.01f:.01f);};
    while(dx!=0&&!supported(dx,0))reduce(dx);
    while(dz!=0&&!supported(0,dz))reduce(dz);
    while((dx!=0||dz!=0)&&!supported(dx,dz)){reduce(dx);reduce(dz);}
    p.velocity.x=dx/dt;p.velocity.z=dz/dt;
  }
  ...
  p.onGround=!p.isFlying&&collides(p,p.position-glm::vec3(0,.03f,0),w);if(!wasGrounded&&p.onGround&&p.gameMode()==GameMode::Survival)p.survival.land();
}
