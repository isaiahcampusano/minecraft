...
bool Inventory::addTo(std::array<ItemStack,N>& slots,const ItemStack& item){
  int capacity=0;for(const auto& stack:slots){if(stack.empty())capacity+=item.kind==ItemKind::TOOL?1:MAX_STACK_SIZE;else if(item.kind!=ItemKind::TOOL&&stack.kind==item.kind&&(item.kind==ItemKind::BLOCK?stack.blockType==item.blockType:stack.materialType==item.materialType))capacity+=MAX_STACK_SIZE-stack.count;}if(capacity<item.count)return false;
  int remaining=item.count;if(item.kind!=ItemKind::TOOL)for(auto& stack:slots){const bool same=stack.kind==item.kind&&(item.kind==ItemKind::BLOCK?stack.blockType==item.blockType:stack.materialType==item.materialType);if(same&&stack.count>0&&stack.count<MAX_STACK_SIZE){const int amount=std::min(remaining,MAX_STACK_SIZE-stack.count);stack.count+=amount;remaining-=amount;if(remaining==0)return true;}}
...
void Inventory::swapCraft(bool table,int i){
  ...
  const bool same=!slot.empty()&&slot.kind==m_cursorStack.kind&&slot.kind!=ItemKind::TOOL&&(slot.kind==ItemKind::BLOCK?slot.blockType==m_cursorStack.blockType:slot.materialType==m_cursorStack.materialType);
