...
enum class Area{NONE,HOTBAR,BACKPACK,CREATIVE,PERSONAL_CRAFT,TABLE_CRAFT,CRAFT_OUTPUT,HELD};
...
inline Hit hitTest(float x,float yFromTop,int width,int height,bool tableOpen=false,bool creativeVisible=true){
  ...
  if(creativeVisible){const float creativeX=paletteX(width);for(int i=0;i<CREATIVE_PAGE_SIZE;++i)if(contains(x,y,creativeX,BACKPACK_Y+i*(PALETTE_HEIGHT+GAP),PALETTE_WIDTH,PALETTE_HEIGHT))return{Area::CREATIVE,i};}
