...
inline std::string itemName(const ItemStack& item){if(item.kind==ItemKind::BLOCK)return blockName(item.blockType);if(item.kind==ItemKind::MATERIAL)return materialName(item.materialType);if(item.kind==ItemKind::FOOD)return foodName(item.foodType);return std::string(toolTierName(item.toolTier))+" "+toolKindName(item.toolKind);}
