...
void Renderer::drawOverlay(int width,int height,const std::string& text,const Inventory& inventory,bool inventoryOpen,bool tableOpen,int creativePage){
  ...
  auto itemTint=[&](const ItemStack& stack){if(stack.kind==ItemKind::BLOCK)return tintFor(stack.blockType);const ItemColor color=stack.kind==ItemKind::MATERIAL?materialColor(stack.materialType):toolColor(stack.toolKind,stack.toolTier);return glm::vec4{color.r/255.f,color.g/255.f,color.b/255.f,1.f};};
...
void Renderer::draw(const World&w,const Player&player,bool showPlayer,float dt,const glm::mat4&v,const glm::mat4&p,const glm::vec3&camera,const RayHit&hit,int width,int height,const std::string&hud,const Inventory&inventory,bool inventoryOpen,bool tableOpen,int creativePage,const DayNightCycle&dayNight){drawSky(dayNight);m_shader.use();m_shader.setMat4("view",v);m_shader.setMat4("projection",p);m_shader.setInt("atlas",0);m_shader.setVec3("cameraPos",camera);m_shader.setVec3("fogColor",dayNight.skyBottom());m_shader.setFloat("daylight",dayNight.daylight());m_texture.bind();w.render();if(showPlayer)m_playerRenderer->draw(player,dt,v,p,camera,dayNight);m_particles.update(dt);m_particles.render(v,p,m_texture,dayNight.daylight());drawOutline(hit,v,p);drawMiningCrack(player.targetedBlock,player.blockBreakProgress,v,p);drawOverlay(width,height,hud,inventory,inventoryOpen,tableOpen,creativePage);}
