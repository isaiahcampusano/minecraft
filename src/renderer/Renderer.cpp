...
void Renderer::drawOverlay(int width,int height,const std::string& text,const Player& player,const Inventory& inventory,bool inventoryOpen,bool tableOpen,int creativePage,float mouseX,float mouseY,int menuState,int viewDistance){
  ...
  if(menuState!=0&&!inventoryOpen){
    ...
    std::vector<float> panel;appendQuad(panel,left,bottom,panelW,panelH);drawQuads(panel,{.08f,.08f,.1f,.97f});
    auto menuText=[&](const std::string& value,float y){drawText(value,left+28.f,y,2.f,{1,1,1,1});};
    if(menuState==1){menuText("PAUSE",bottom+panelH-42.f);menuText("RESUME",bottom+panelH-100.f);menuText("SETTINGS",bottom+panelH-150.f);menuText("SAVE QUIT",bottom+panelH-200.f);}
    else if(menuState==2){menuText("SETTINGS",bottom+panelH-42.f);menuText("GAME MODE",bottom+panelH-100.f);menuText(std::string("VIEW DISTANCE ")+std::to_string(viewDistance),bottom+panelH-150.f);menuText("KEYBINDS",bottom+panelH-200.f);menuText("BACK",bottom+panelH-250.f);}
    else {menuText("KEYBINDS",bottom+panelH-42.f);menuText("PRESS A KEY TO REBIND",bottom+panelH-100.f);menuText("RESET CONTROLS",bottom+panelH-160.f);menuText("BACK",bottom+panelH-220.f);}
  }
