...
void Renderer::draw(const World&w,const Player&player,const PassiveMobSystem&mobs,bool showPlayer,float dt,const glm::mat4&v,const glm::mat4&p,const glm::vec3&camera,const RayHit&hit,int width,int height,const std::string&hud,const Inventory&inventory,bool inventoryOpen,bool tableOpen,int creativePage,float mouseX,float mouseY,const DayNightCycle&dayNight){drawSky(dayNight);m_shader.use();m_shader.setMat4("view",v);m_shader.setMat4("projection",p);m_shader.setInt("atlas",0);m_shader.setVec3("cameraPos",camera);m_shader.setVec3("fogColor",dayNight.skyBottom());m_shader.setFloat("daylight",dayNight.daylight());m_texture.bind();w.render();if(showPlayer)m_playerRenderer->draw(player,dt,v,p,camera,dayNight);else m_playerRenderer->drawFirstPerson(player,v,p,camera,dayNight);m_mobRenderer->draw(mobs,w,v,p,camera,dayNight);m_particles.update(dt);m_particles.render(v,p,m_texture,dayNight.daylight());m_drops.render(v,p,m_texture,dayNight.daylight());drawOutline(hit,v,p);drawMiningCrack(player.targetedBlock,player.blockBreakProgress,v,p);drawOverlay(width,height,hud,player,inventory,inventoryOpen,tableOpen,creativePage,mouseX,mouseY);}
