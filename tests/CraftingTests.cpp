...
int main(){
  int toolRecipes=0;std::array<std::array<bool,static_cast<std::size_t>(ToolTier::COUNT)>,static_cast<std::size_t>(ToolKind::COUNT)> toolSeen{};for(const auto& recipe:CraftingRegistry::recipes())if(recipe.output.kind==ItemKind::TOOL){++toolRecipes;toolSeen[static_cast<std::size_t>(recipe.output.toolKind)][static_cast<std::size_t>(recipe.output.toolTier)]=true;if(!recipe.requiresTable())return fail("tool recipe did not require the crafting table");}if(CraftingRegistry::recipes().size()!=14||toolRecipes!=10)return fail("initial recipe registry did not contain 4 material and 10 tool recipes");for(const auto& tiers:toolSeen)for(bool found:tiers)if(!found)return fail("a tool kind/tier recipe was missing");
  ...
  grid={};for(int i=0;i<3;++i){grid[i]=ItemStack::material(MaterialType::WHITE_WOOL);grid[i+3]=wood;}const auto* bed=CraftingRegistry::match(grid,3,3);if(!bed||bed->output.blockType!=BlockType::BED)return fail("bed recipe did not match wool over planks");
