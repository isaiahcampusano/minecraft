...
int main(){
  int toolRecipes=0;std::array<std::array<bool,static_cast<std::size_t>(ToolTier::COUNT)>,static_cast<std::size_t>(ToolKind::COUNT)> toolSeen{};for(const auto& recipe:CraftingRegistry::recipes())if(recipe.output.kind==ItemKind::TOOL){++toolRecipes;toolSeen[static_cast<std::size_t>(recipe.output.toolKind)][static_cast<std::size_t>(recipe.output.toolTier)]=true;if(!recipe.requiresTable())return fail("tool recipe did not require the crafting table");}if(CraftingRegistry::recipes().size()!=15||toolRecipes!=10)return fail("initial recipe registry did not contain 5 material and 10 tool recipes");for(const auto& tiers:toolSeen)for(bool found:tiers)if(!found)return fail("a tool kind/tier recipe was missing");
