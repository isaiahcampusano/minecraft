...
bool sameMob(const MobData& a,const MobData& b){return a.id==b.id&&a.type==b.type&&a.x==b.x&&a.y==b.y&&a.z==b.z&&a.yaw==b.yaw&&a.health==b.health&&a.age==b.age&&a.grazeCooldown==b.grazeCooldown;}
...
int main(){
  ...
  source.hotbar[0]=toolSlot(ToolKind::PICKAXE,ToolTier::WOOD,17);source.hotbar[1]=toolSlot(ToolKind::AXE,ToolTier::STONE,129);source.hotbar[2]=foodSlot(FoodType::RAW_BEEF,11);source.backpack[3]=materialSlot(MaterialType::LEATHER,12);source.health=13;source.hunger=16;source.saturation=4.5f;source.exhaustion=3.25f;source.mobs={{41,MobType::COW,501.5f,7.f,502.5f,.4f,7,0,12.f},{42,MobType::SHEEP,511.5f,7.f,512.5f,-.7f,5,-600.f,3.f}};
  ...
  if(loaded.selectedSlot!=source.selectedSlot||!sameSlot(loaded.cursorStack,source.cursorStack)||loaded.edits.size()!=source.edits.size()||loaded.mobs.size()!=source.mobs.size()||loaded.health!=13||loaded.hunger!=16||loaded.saturation!=4.5f||loaded.exhaustion!=3.25f)return fail("top-level save data did not round-trip");
  ...
  for(std::size_t i=0;i<source.mobs.size();++i)if(!sameMob(loaded.mobs[i],source.mobs[i]))return fail("mob data did not round-trip");
  ...
  source.mobs.push_back(source.mobs.front());if(SaveLoad::save(source,directory/"duplicate-mob.dat"))return fail("duplicate mob ids were serialized");source.mobs.pop_back();source.mobs[0].x=std::numeric_limits<float>::quiet_NaN();if(SaveLoad::save(source,directory/"invalid-mob.dat"))return fail("invalid mob coordinates were serialized");source.mobs[0].x=501.5f;
  ...
  const auto v3Path=directory/"migration-v3.dat";writeV3Save(v3Path,18,17,4.f,1.f);SaveData migratedV3;if(!SaveLoad::load(migratedV3,v3Path)||migratedV3.health!=18||!migratedV3.mobs.empty())return fail("MCv3 migration did not default to an empty mob list");
