...
int main(){
  ...
  Player sourcePlayer;sourcePlayer.survival.restore(14,17,3.5f,2.25f);const SaveData data=GameState::capture(sourceInventory,sourceWorld,sourcePlayer);if(data.edits.size()!=3)return fail("capture did not include every sparse world edit");
  Inventory restoredInventory;World restoredWorld;Player restoredPlayer;GameState::apply(data,restoredInventory,restoredWorld,restoredPlayer);
